...
墨水屏版式（2026-09 v5 定稿）：
  横图（宽≥高）或无图 → 现版式：顶部32px信息栏（描述1行截断|大区域|小区域）+ 横线 + 大照片
  竖图（宽<高）      → 全自适应新版式：
      照片高300贴满、宽=300×宽高比、左贴边；
      竖线紧贴照片右缘；剩余宽度全部归文字列；
      描述按列宽自动换行 1~3 行（3行封顶，超出直接切，无省略号）→ 线 →
      大区域1行 → 线 → 小区域1行 → 线；
      文字块从顶部6px开始，全部18px同字重，墨迹垂直居中于24px行槽中线。

渲染预览（验收用，不碰真实数据）：
  系统配置表加一行「渲染预览」，值 = 竖 / 竖短 / 竖方 / 横
  → 跑 workflow 会用示例数据渲染对应版式并推到屏幕，
    不发飞书消息、不动任务字段，跑完自动把值改回「否」。
  值 = 否 或无此行 = 正常模式。
...
def list_records(table_id, view_id=None, page_size=200):
    ...
    while True:
        ...
        result = api_request("GET",
            f"/bitable/v1/apps/{BASE_TOKEN}/tables/{table_id}/records", params=params)
        if result.get("code") != 0:
            break
        ...
        if os.path.exists(p):
            try:
                return ImageFont.truetype(p, size,
                    index=2 if p.endswith(".ttc") else 0)
            except Exception:
                continue
...
def _draw_row(draw, x, y_top, w, text, font, color=INK):
    """24px 行槽内绘制，墨迹垂直居中于行槽几何中线（textbbox 校准）"""
    if not text:
        return
    bbox = font.getbbox(text)
    ty = y_top + 12 - (bbox[3] - bbox[1]) / 2 - bbox[1]
...
# ---- 横图版式（现版式，一个像素不动） ----
def render_landscape(img, draw, desc, big, small, photo):
    draw.text((6, _info_y(desc and _truncate(desc, FONT, 214))), "",
              font=FONT)  # 占位防警告
    ...
    if photo:
        ...
        p = fit_contain(photo, 400, 262)
        img.paste(p, (round((400 - p.width) / 2), 34 + round((262 - p.height) / 2)))
    else:
        ph = "这里是图片"
        w = FONT_PLACEHOLDER.getlength(ph)
        bbox = FONT_PLACEHOLDER.getbbox(ph)
        ty = 34 + 131 - (bbox[3] - bbox[1]) / 2 - bbox[1]
...
# ---- 竖图版式（v5 全自适应） ----
def render_portrait(img, draw, desc, big, small, photo):
    H = 300
...
def render_eink_image(task, photo=None):
    ...
    if photo and photo.width < photo.height:
        ...
        print(f"版式: 横图现版式（{ratio}）")
        render_landscape(img, draw, desc, big, small, photo)
    return img
...
def run_preview(mode):
    ...
    img = render_eink_image({"具体区域描述": desc, "大区域": big, "小区域": small}, photo)
...
def refresh_eink_only(args):
    """防重复触发时：只刷新墨水屏图，不发消息"""
    records = list_records(TASK_TABLE_ID, DEFAULT_VIEW_ID)
    todo = [r for r in records if is_valid_task(r["fields"])
            and r["fields"].get("是否今日")]
    today_view_id = find_view_id_by_name(TASK_TABLE_ID, TODAY_VIEW_NAME)
    if today_view_id:
        ordered = [r["record_id"] for r in list_records(TASK_TABLE_ID, today_view_id)]
        todo.sort(key=lambda r: ordered.index(r["record_id"]) if r["record_id"] in ordered else 9999)
...
def refresh_eink_display(todo):
    ...
    if not target and todo:
        target = todo[0]
    if not target:
        img = render_eink_image({"具体区域描述": "（无任务）", "大区域": "", "小区域": ""}, None)
    else:
        photo = fetch_photo(target["fields"])
        img = render_eink_image(target["fields"], photo)
    os.makedirs("docs", exist_ok=True)
    img.save("docs/today.png")
    push_photo_to_funnycoo(img)
...
def ensure_log_table_and_write(done_records):
    result = api_request("POST",
        f"/bitable/v1/apps/{BASE_TOKEN}/tables/search",
        json_body={})
    log_table_id = None
    if result.get("code") == 0:
    ...
    if not log_table_id:
        r = api_request("POST",
            ...
            json_body={"table": {"name": LOG_TABLE_NAME,
                "default_view_name": "记录",
                "fields": [
    ...
    midnight = int(datetime.now(BEIJING_TZ).replace(hour=0, minute=0,
    ...
    for r in done_records:
        ...
        big = f.get("大区域")
        small = f.get("小区域")
        if isinstance(big, list): big = big[0] if big else ""
        if isinstance(small, list): small = small[0] if small else ""
    ...
    if new_records:
        api_request("POST",
            f"/bitable/v1/apps/{BASE_TOKEN}/tables/{log_table_id}/records/batch_create",
